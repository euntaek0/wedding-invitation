...
function uploadViaTarget(file: File, target: UploadTarget, onProgress: (progress: number) => void) {
    ...
    try {
      ...
      for (const candidate of candidates) {
        ...
        if (!target) {
          ...
          setItems((prev) =>
            prev.map((item) =>
              item.id === candidate.id ? { ...item, status: "error", error: copy.missingTarget } : item,
            ),
          );
          continue;
        }
        ...
        setItems((prev) =>
          prev.map((item) =>
            item.id === candidate.id ? { ...item, status: "uploading", progress: 0, error: undefined } : item,
          ),
        );

        try {
          await uploadViaTarget(candidate.file, target, (progress) => {
            setItems((prev) => prev.map((item) => (item.id === candidate.id ? { ...item, progress } : item)));
          });
          ...
          setItems((prev) =>
            prev.map((item) => (item.id === candidate.id ? { ...item, status: "error", error: message } : item)),
          );
        }
      }
    ...
    <Reveal className="wi-section wi-section-upload px-5 py-12 sm:px-8 sm:py-14">
      <section id="guest-upload" className="wi-upload space-y-6">
        ...
        <div className="space-y-2 text-center">
          <p className="wi-upload-description whitespace-pre-line text-base leading-relaxed text-[var(--muted)]">
            {copy.description}
          </p>
        </div>
        ...
        <button
          ...
          className="wi-upload-open-button wi-btn-primary mx-auto mt-6 flex min-w-[180px] items-center justify-center gap-2 rounded-xl px-4 py-4 text-base font-medium"
        >
      ...
      <Modal open={isOpen} onClose={() => setIsOpen(false)} title={copy.heading} closeLabel={copy.closeModal}>
        <div className="wi-upload-form space-y-4">
                ...
                <li key={item.id} className="wi-upload-item rounded-xl border border-[var(--line)] p-2">
                  <div className="flex gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-[var(--muted)]">{(item.file.size / 1024 / 1024).toFixed(2)}MB</p>
          ...
          {feedback && (
            <p
              className={`wi-upload-feedback text-sm ${feedback.type === "success" ? "text-emerald-600" : "text-rose-500"}`}
            >
              {feedback.text}
            </p>
          )}
